...
extension ChatService {
    ...
    func updateWorkspaceChatSettings(workspaceId: UUID, permissions: ChatWorkspacePermissions) async throws -> Workspace {
        ...
        let existing: Workspace = try await client
            .from("workspaces")
            .select()
        ...
        struct Patch: Encodable { let settings: [String: JSONValue] }
        return try await client
            .from("workspaces")
    ...
    func subscribeMessageUpdates(
        ...
        onDelete: @escaping @Sendable (UUID, UUID) -> Void
    ) {
        Task {
            ...
            await withTaskGroup(of: Void.self) { group in
                group.addTask {
                    for await action in updates {
                        guard let record = try? action.decodeRecord(as: ChatMessage.self, decoder: decoder) else { continue }
                        onUpdate(record)
                    }
                }
                group.addTask {
                    for await action in deletes {
                        guard let old = try? action.decodeOldRecord(as: ChatMessage.self, decoder: decoder) else { continue }
                        onDelete(old.id, old.channelId)
                    }
                }
            }
        }
