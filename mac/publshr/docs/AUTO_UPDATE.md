...
3. Your installed app checks the `live` channel every **minute** via `VERSION.txt` (and when the app becomes active or wakes from sleep). It compares **build number**, **full version**, **git commit**, and **package digest**, then downloads the complete app bundle (icons, colors, Swift UI, features), replaces **the running app in place**, and **restarts automatically** when auto-install is on. Installs to `~/Applications/Publshr.app` are **passwordless**; system `/Applications` uses **one** administrator prompt per update (not three).
