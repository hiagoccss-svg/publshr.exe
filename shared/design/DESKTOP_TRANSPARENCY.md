...
- Native window effects: `shared/desktop/TAURI_GLASS.md` per app `src-tauri`
- Set `data-glass-theme="light"` or `"dark"` on `<html>`

## Legacy Electron

- Shared window helper: `shared/electron/glass-window.ts`
- Maintenance-only; do not add new Electron shells (see `desktop/docs/TAURI_DESKTOP.md`)

### Apps

| App | Theme | Shell |
|-----|-------|-------|
| Spaces | light | Tauri (`desktop/spaces/src-tauri`) |
| Planner | light | Electron (`planner/desktop/src/main/index.ts`) |
| Media Monitoring | dark | Electron (`desktop/media-monitoring/electron/main/index.ts`) |
